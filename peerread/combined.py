...
parser.add_argument("--model", type=str, default='combined', choices=['combined', 'combined-no-dg'], help="Model type (default=combined)")
...
dataset, input_shape = token_encode(data_x_paragraph, data_y_paragraph,
...
for training_seed in range(42, 52):
    ...
    main_model_checkpoint = './results/bert_reg/{}/model_{}.pt'.format(args.training_ratio, training_seed)
    if args.model == 'combined':
        coh_checkpoint = '../discourse/results/best_model.pt'
    else:
        coh_checkpoint = None
    ...
    result_dir = './results/{}/{}'.format(args.model, args.training_ratio)
    ...
    for batch_size, lr in gen_hp():
        ...
        model = CombinedModel(input_shape, main_model_checkpoint=main_model_checkpoint, coh_checkpoint=coh_checkpoint)
        model.cuda()
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                b_s_input_ids = batch[3].to(device)
                b_s_input_type_ids = batch[4].to(device)
                b_s_input_mask = batch[5].to(device)
                ...
                (loss, _, _) = model(b_input_ids, 
                                    ...
                                    s_x=b_s_input_ids,
                                    s_token_type_ids=b_s_input_type_ids,
                                    s_attention_mask=b_s_input_mask,
                                    labels=b_labels)
